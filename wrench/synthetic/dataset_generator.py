...
class ConditionalIndependentGenerator(BaseSyntheticGenerator):
    def __init__(
        ...
        self.alpha_l = self.generator.uniform(
            low=max(0, alpha - alpha_radius),
            high=min(1, alpha + alpha_radius),
            size=n_lfs,
        )
        self.beta_l = self.generator.uniform(
            low=max(0, beta - beta_radius), high=min(1, beta + beta_radius), size=n_lfs
        )
    ...
    def generate(self, n_data: int = 1000):
        ...
        labels = list(
            self.generator.choice(self.n_class, size=n_data, p=self.class_prior)
        )
        weak_labels = []
...
class DataDependentGenerator(ConditionalIndependentGenerator):
    def __init__(
        ...
        self.lf_pro_clusters = [
            self.generator.choice(n_cluster, size=n_cluster_per_lfs)
    ...
    def generate(self, n_data: int = 1000):
        ...
        for i, y in enumerate(labels):
            weak_label = []
            cluster = clusters[i]
            for alpha, beta, gamma, target, lf_pro_clusters in zip(
                self.alpha_l,
                self.beta_l,
                self.gamma_l,
                self.lf_targets,
...
class CorrelatedGenerator(ConditionalIndependentGenerator):
    def __init__(
        ...
        self.conflict_lfs = self.generator.choice(
            lf_pool, size=n_conflict, replace=False
        )
        lf_pool = [i for i in lf_pool if i not in self.conflict_lfs]
        self.duplicate_lfs = self.generator.choice(
            lf_pool, size=n_duplicate, replace=False
        )
        self.normal_lfs = [i for i in lf_pool if i not in self.duplicate_lfs]
        self.overlap_target_lf = self.generator.choice(
            self.normal_lfs, size=n_overlap, replace=False
        )
        self.conflict_target_lf = self.generator.choice(
            self.normal_lfs, size=n_conflict, replace=False
        )
        self.duplicate_target_lf = self.generator.choice(
            self.normal_lfs, size=n_duplicate, replace=False
        )
        self.dep_graph = []
        ...
        for conflict_lf, target in zip(self.conflict_lfs, self.conflict_target_lf):
            self.lf_targets[conflict_lf] = self.sample_other_label(
                self.lf_targets[target]
            )
                ...
                if weak_label[m] != -1:
                    ...
                    if target == y:
                        ...
                        p = (
                            (1 - alpha)
                            * beta
                            / (self.class_prior[y] * (self.n_class - 1))
                        )
                    if self.generator.random() < p:
                        weak_label[j] = target
