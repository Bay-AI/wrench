...
class BaseSeqDataset:
    ...
    def save_embed_dict(
        self,
        save_word_dict_path: Optional[str] = None,
        save_char_dict_path: Optional[str] = None,
    ):
...
def build_bert_embeddings(corpus, bert_model_name, device):
    ...
    for i, sents in enumerate(corpus):
        ...
        if len_bert_tokens >= 510:
            sents_list = [sents]
            bert_length_list = [
                len(tokenizer.tokenize(" ".join(t))) for t in sents_list
            ]
    ...
    for i, sent in enumerate(
        tqdm(standarized_sents, desc="extracting bert embedding...")
    ):
        joint_sent = " ".join(sent)
        bert_tokens = tokenizer.tokenize(joint_sent)

        input_ids = torch.tensor(
            [tokenizer.encode(joint_sent, add_special_tokens=True)], device=device
        )
        ...
        ori2bert, bert2ori = get_alignments(
            sent, bert_tokens
        )  # there is an error that get_alignments func. cannot recognize [UNK] well
