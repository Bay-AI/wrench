...
bin_cls_dataset_list = numeric_datasets + [
    "cdr",
    "spouse",
    "sms",
    "yelp",
...
def get_dataset_type(dataset_name):
    ...
    raise NotImplementedError(
        "cannot recognize the dataset type! please specify the dataset_type."
    )
...
def load_dataset(
    ...
    **kwargs,
    ...
    if extract_feature and (dataset_class != BaseSeqDataset):
        extractor_fn = train_data.extract_feature(
            extract_fn=extract_fn, return_extractor=True, **kwargs
        )
        valid_data.extract_feature(
            extract_fn=extractor_fn, return_extractor=False, **kwargs
        )
        test_data.extract_feature(
            extract_fn=extractor_fn, return_extractor=False, **kwargs
        )
