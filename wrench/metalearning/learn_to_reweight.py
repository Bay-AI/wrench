            ...
            with trange(
                ...
                for batch in train_dataloader:
                    ...
                    with higher.innerloop_ctx(model, optimizer, device=device) as (
                        ...
                        meta_train_loss = cross_entropy_with_probs(
                            meta_logits, target, reduction="none"
                        )
                        eps = torch.zeros(
                            len(target), requires_grad=True, device=device
                        )
                        ...
                        meta_loss = cross_entropy_with_probs(
                            meta_model(meta_batch), meta_batch["labels"].to(device)
                        )
                    ...
                    loss = torch.sum(
                        ...
                        * cross_entropy_with_probs(
                            weak_logits, target, reduction="none"
                        )
                    )
