...
class Encoder(BackBone):
    def __init__(
        ...
        if balance is None:
            self.log_class_prior = nn.Parameter(
                torch.log(torch.ones(n_class) / n_class), requires_grad=False
            )
        else:
            self.log_class_prior = nn.Parameter(
                torch.from_numpy(balance), requires_grad=False
            )
    ...
    def forward(self, batch):
        ...
        z = (
            self.encoder(x).view(batch_size, self.n_rules, self.n_class)
            / self.temperature
        )
...
class WeaSELModel(BackBone):
    def __init__(
        ...
        self.encoder = Encoder(
            input_size, n_rules, hidden_size, n_class, temperature, dropout, balance
        )
    ...
    def calculate_loss(self, batch):
        ...
        loss_f = cross_entropy_with_probs(
            predict_f, torch.softmax(predict_e.detach(), dim=-1)
        )
        loss_e = cross_entropy_with_probs(
            predict_e, torch.softmax(predict_f.detach(), dim=-1)
        )
        loss = loss_e + loss_f
        return loss
