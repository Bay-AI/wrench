...
class RuleAttentionTeacherNetwork(BackBone):
    ...
    def forward(self, batch, features, proba):
        ...
        uniform_weight = (
            ...
            + 1
            - student_attention
            - torch.sum(rule_attention, dim=1, keepdim=True)
        )
        ...
        weighted_sum = (
            torch.sum(weighted_rule, dim=1) + weighted_proba + weighted_uniform
        )
...
class AstraModel(BackBone):
    def __init__(self, rule_embed_size, dropout, n_rules, n_class, backbone):
        ...
        self.ran = RuleAttentionTeacherNetwork(
            rule_embed_size, n_rules, n_class, backbone.hidden_size, dropout
        )
...
class Astra(BaseTorchClassModel):
    ...
    def fit(
        ...
        **kwargs: Any,
        ...
        labeled_dataset, unlabeled_dataset = dataset_train.create_split(
            labeled_data_idx
        )
        ...
        if pretrained_model is not None:
            ...
            optimizer, scheduler = self._init_optimizer_and_lr_scheduler(
            ...
            with trange(
                n_steps,
                desc="[TRAIN] ASTRA pretrain student",
                unit="steps",
                disable=not verbose,
                ncols=150,
                position=0,
                ...
                for batch in labeled_dataloader:
                    ...
                    loss = cross_entropy_with_probs(
                        predict_l, batch["labels"].to(device)
                    )
                    loss.backward()
                    cnt += 1
        ...
        for i in range(n_iter):
            ...
            pseudo_probas_u, features_u = self.collect_pseudodataset_student(
                unlabeled_dataset
            )
            pseudo_probas_l, features_l = self.collect_pseudodataset_student(
                labeled_dataset
            )
            ...
            with trange(
                ...
                for unlabeled_batch in unlabeled_dataloader:
                    ...
                    predict_u = model.forward_teacher(
                        unlabeled_batch, features_u[idx_u], pseudo_probas_u[idx_u]
                    )
                    loss = -torch.mean(
                        torch.sum(predict_u * torch.log(predict_u), dim=-1)
                    )
                    loss.backward()
                    cnt += 1
                    if cnt % accum_steps == 0:
            ...
            with trange(
                n_steps,
                desc=f"[FINETUNE@{i}] ASTRA-teacher",
                unit="steps",
                disable=not verbose,
                ncols=150,
                position=0,
                ...
                for label_batch in labeled_dataloader:
                    ...
                    predict_l = model.forward_teacher(
                        label_batch, features_l[idx_l], pseudo_probas_l[idx_l]
                    )
                    loss = cross_entropy_with_probs(
                        predict_l, label_batch["labels"].to(device)
                    )
                    loss.backward()
                    cnt += 1
                    if cnt % accum_steps == 0:
            ...
            with trange(
                n_steps,
                desc=f"[FINETUNE@{i}] ASTRA-student",
                unit="steps",
                disable=not verbose,
                ncols=150,
                position=0,
            ...
            if valid_flag:
                ...
                if (
                    ...
                    and metric_value > outer_best_metric_value
                ) or (
