...
class AssembleModel(BackBone):
    def __init__(self, input_size, n_rules, hidden_size, n_class, backbone):
        ...
        self.attention = AttentionModel(
            input_size + n_rules, n_rules, hidden_size, n_class
        )  # TODO
    ...
    def forward(self, batch_l, batch_u, x_lf_l, x_lf_u):
        ...
        fix_score = F.softmax(
            torch.mean(all_scores, dim=0), dim=0
        )  # use the average as the fixed score

        lf_y_u = torch.zeros(
            (x_lf_u.size(0), self.n_class), dtype=torch.float, device=self.get_device()
        )
        for k in range(self.n_class):
            lf_y_u[:, k] = (
                fix_score.unsqueeze(0).repeat([x_lf_u.size(0), 1])
                * (x_lf_u == k).float()
            ).sum(dim=1)
...
class Denoise(BaseTorchClassModel):
    ...
    def fit(
        ...
        **kwargs: Any,
        ...
        label_model.fit(
            dataset_train=dataset_train, dataset_valid=dataset_valid, verbose=verbose
        )
        self.label_model = label_model
        all_y_l = torch.LongTensor(label_model.predict(labeled_dataset)).to(device)
        all_Z = torch.zeros(len(unlabeled_dataset), n_class, dtype=torch.float).to(
            device
        )
        all_z = torch.zeros(len(unlabeled_dataset), n_class, dtype=torch.float).to(
            device
        ...
        try:
            with trange(
                ...
                for labeled_batch, unlabeled_batch in zip(
                    labeled_dataloader, unlabeled_dataloader
                ):
                    ...
                    predict_l, predict_u, lf_y_l, lf_y_u, fix_score = model(
                        labeled_batch, unlabeled_batch, x_lf_l, x_lf_u
                    )
                    ...
                    if step > 100:
                        ...
                        all_z[idx_u] = Z * (1.0 / (1.0 - alpha ** (step + 1)))
                        all_Z[idx_u] = Z
                    ...
                    if cnt % accum_steps == 0:
                        ...
                        # --- update y_l ---
                        lf_y_l_new = torch.zeros(
                            (x_lf_l.size(0), n_class), dtype=torch.float
                        ).to(device)
                        for k in range(n_class):
                            lf_y_l_new[:, k] = (
                                fix_score.unsqueeze(0).repeat([x_lf_l.size(0), 1])
                                * (x_lf_l == k).float()
                            ).sum(dim=1)
                        lf_y_l_new /= torch.sum(lf_y_l_new, dim=1).unsqueeze(1)
                        lf_y_l_new[lf_y_l_new != lf_y_l_new] = (
                            0  # handle the 'nan' (divided by 0) problem
                        )
        ...
        for batch in valid_dataloader:
            if mode == "ensemble":
                ...
                mask = torch.unsqueeze(
                    (max_prob_feature > max_prob_weak_labels).long(), dim=1
                )
