...
class ImplyLossModel(BackBone):
    def __init__(
        self, input_size, n_rules, n_class, backbone, hidden_size, q, dropout=0.8
    ):
    ...
    def forward(self, batch):
        ...
        if torch.any(covered_mask):
            ...
            covered_features = torch.repeat_interleave(
                covered_features, self.n_rules, dim=0
            )
            ...
            weak_labels_one_hot = F.one_hot(
                covered_weak_labels_list * fire_mask, num_classes=self.n_class
            )
            score = weak_labels_one_hot * r_score.unsqueeze(2) + (
                1 - weak_labels_one_hot
            ) * (1 - r_score.unsqueeze(2))
            score_mean = torch.sum(score * fire_mask.unsqueeze(2), dim=1) / torch.sum(
                fire_mask, dim=1, keepdim=True
            )
    ...
    def calculate_labeled_batch_loss(self, labeled_batch, data_exemplar_matrix):
        ...
        loss_phi_1 = F.binary_cross_entropy(
            r_score_l, torch.ones_like(r_score_l), reduction="mean"
        )
        ...
        if torch.sum(mask):
            ...
            loss_phi_2 = F.binary_cross_entropy(
                r_score_l, torch.zeros_like(r_score_l), reduction="mean"
            )
        else:
            loss_phi_2 = 0.0
    ...
    def fit(
        ...
        **kwargs: Any,
        ...
        unlabeled_train_dataloader = self._init_train_dataloader(
            unlabeled_dataset, n_steps=n_steps, config=config
        )
        ...
        try:
            with trange(
                ...
                for unlabeled_batch, labeled_batch in zip(
                    unlabeled_train_dataloader, labeled_train_dataloader
                ):
