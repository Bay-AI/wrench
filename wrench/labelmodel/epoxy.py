...
class Epoxy:
    ...
    def __init__(
        ...
        if self.method == "faiss":
            ...
            if gpu:
                ...
                label_fn_indexes = [
                    faiss.index_cpu_to_gpu(res, gpuid, x) for x in label_fn_indexes
                ]
    ...
    def preprocess(self, L_mat, mat_embeddings, batch_size=None):
        """
        ...
        if self.method == "faiss":
            mat_abstains = [
                np.argwhere(L_mat[:, i] == -1).flatten() for i in range(self.m)
            ]
            ...
            def comp_similarity(embs):
                if self.metric == "cosine":
                    if self.method == "sklearn":
                        ...
                        if self.gpu:
                            return (
                                pytorch_cosine_similarity(
                                    torch.tensor(embs).type(torch.float16).cuda(),
                                    torch.tensor(self.train_embeddings)
                                    .type(torch.float16)
                                    .cuda(),
                                )
                                .cpu()
    ...
    def extend(self, thresholds):
        ...
        if self.method == "faiss":
            ...
            new_points = [
                ...
                if self.metric == "cosine"
                else (self.dists[i] < thresholds[i])
                for i in range(self.m)
            ]

            for i in range(self.m):
                expanded_L_mat[self.mat_abstains[i][new_points[i]], i] = self.L_train[
                    self.support[i], i
...
def preprocess_lfs(
    ...
    for l in range(n_class):
        train_support = [np.argwhere(L_train[:, i] == l).flatten() for i in range(m)]
        ...
        closest = [
            ...
            if dists[i].shape[1] > 0
            else np.full(mat_abstains[i].shape, -1)
            for i in range(m)
        ]
...
def extend_lfs(L_mat, mat_abstains, closest_l, thresholds, metric="cosine"):
    """
    Preprocessing for sklearn method.
    ...
    if metric == "cosine":
        max_dist_l = []
        for i in range(m):
            max_dist = [closest[i] for closest in closest_l]
        ...
        min_dist_l = []
        for i in range(m):
            min_dist = [closest[i] for closest in closest_l]
