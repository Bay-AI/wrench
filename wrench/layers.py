...
class CharBiLSTM(nn.Module):
    def __init__(
        ...
        if pretrain_char_embedding is not None:
            ...
            self.char_embeddings.weight.data.copy_(
                torch.from_numpy(random_embedding(char_vocab_size, char_emb_dim))
            )
    ...
    def forward(self, input, seq_lengths):
        ...
        pack_input = pack_padded_sequence(
            char_embeds, seq_lengths, batch_first=True, enforce_sorted=False
        )
...
class CharCNN(nn.Module):
    def __init__(
        ...
        self.char_cnn = nn.Conv1d(
            char_emb_dim, char_hidden_dim, kernel_size=3, padding=1
        )
    ...
    def __init__(
        ...
        if word_embedding is not None:
            ...
            self.word_embedding.weight.data.copy_(
                torch.from_numpy(random_embedding(word_vocab_size, word_emb_dim))
            )
    ...
    def forward(self, word_inputs, char_inputs=None, char_seq_lengths=None):
        ...
        if self.use_char:
            ...
            if self.char_all_feature:
                char_features_extra = self.char_feature_extra(
...
class WordSequence(nn.Module):
    def __init__(
        ...
        if word_feature_extractor == "GRU":
            self.lstm = nn.GRU(
                input_size,
                lstm_hidden,
                num_layers=n_word_hidden_layer,
                batch_first=True,
                bidirectional=True,
            )
    ...
    def forward(
        self, word_inputs, word_seq_lengths, char_inputs=None, char_seq_lengths=None
    ):
        """
        input:
            word_inputs: (batch_size, sent_len)
        ...
        packed_words = pack_padded_sequence(
            word_represent, word_seq_lengths, batch_first=True, enforce_sorted=False
        )
