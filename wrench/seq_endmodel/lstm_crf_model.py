...
def batchify(word_ids_list, char_ids_list, word_dict: Dict, char_dict: Dict):
    ...
    pad_chars = [
        char_ids + [[char_pad_id]] * (max_seq_len - len(char_ids))
...
class LSTMTorchSeqDataset(Dataset):
    def __init__(self, dataset: BaseSeqDataset, n_data: Optional[int] = 0):
        ...
        word_ids_list, char_ids_list = encode(
            dataset.examples, word_dict=dataset.word_dict, char_dict=dataset.char_dict
        )
        (
            word_seq_tensor,
            word_seq_lengths,
    ...
    def fit(
        ...
        **kwargs: Any,
        ...
        torch_dataset = LSTMTorchSeqDataset(
            dataset_train, n_data=n_steps * hyperparas["batch_size"]
        )
        ...
        y_train_pad = (
            np.zeros((len(y_train), torch_dataset.max_seq_len))
            * dataset_train.word_dict[dataset_train.PAD]
        )
