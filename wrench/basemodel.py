...
from .utils import (
    get_bert_torch_dataset_class,
    construct_collate_fn_trunc_pad,
    get_bert_model_class,
)
...
class BaseTorchModel(BaseModel, ABC):
    ...
    def _init_optimizer_and_lr_scheduler(
        self,
        model: nn.Module,
        config: Config,
        ...
        if hasattr(config, "lr_scheduler_config"):
            ...
            if lr_scheduler == "default":
                lr_scheduler_ = get_linear_schedule_with_warmup(
                    optimizer_,
                    num_warmup_steps=0,
                    num_training_steps=config.hyperparas["n_steps"],
                )
            else:
                lr_scheduler_config = config.lr_scheduler_config["paras"]
                lr_scheduler_ = getattr(optim.lr_scheduler, lr_scheduler)(
                    optimizer_, **lr_scheduler_config
                )
        else:
            lr_scheduler_ = None
    ...
    def _init_valid(
        ...
        **kwargs: Any,
        ...
        assert direction in ["maximize", "minimize"], (
            'direction should be in ["maximize", "minimize"]!'
        )
        ...
        if (self.direction == "maximize" and metric_value > self.best_metric_value) or (
            ...
            if self.tolerance > 0 and (
                (
                    ...
                    and metric_value + self.tolerance < self.best_metric_value
                )
                or (
...
class BaseSeqModel(BaseModel, ABC):
    ...
    def test(
        ...
        metric_fn: Union[Callable, str],
        y_true: Optional[List[List]] = None,
...
def check_bert_model(model):
    if isinstance(model, DDP):
        model = model.module
    return isinstance(model, BERTBackBone) or (
        hasattr(model, "backbone") and isinstance(model.backbone, BERTBackBone)
    )
...
def check_vision_model(model):
    if isinstance(model, DDP):
        model = model.module
    return isinstance(model, ImageClassifier) or (
        hasattr(model, "backbone") and isinstance(model.backbone, ImageClassifier)
    )
    ...
    def _init_dataloader(
        ...
        **kwargs: Any,
        ...
        if check_bert_model(self.model):
            ...
            dataloader = DataLoader(
                torch_dataset, batch_size=real_batch_size, shuffle=shuffle, **kwargs
            )
        return dataloader
...
class BaseTorchSeqModel(BaseSeqModel, BaseTorchModel, ABC):
    def _init_label_model(
        ...
        if hasattr(seq_labelmodel, label_model):
            ...
            label_model_ = SeqLabelModelWrapper(
                label_model_class=getattr(labelmodel, label_model), **label_model_config
            )
        return label_model_
    ...
    def _init_valid_step(
        ...
        if isinstance(metric, Callable):
            ...
            metric_fn = partial(
                SEQ_METRIC[metric], id2label=dataset_valid.id2label, strict=strict
            )
        self.metric_fn = metric_fn
    ...
    def predict(
        self,
        dataset: Union[BaseSeqDataset, DataLoader],
        device: Optional[torch.device] = None,
        **kwargs: Any,
    ):
        if device is not None:
