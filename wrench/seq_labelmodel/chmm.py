...
class CHMMTorchSeqDataset(Dataset):
    def __init__(self, dataset: BaseSeqDataset, for_train=True):
        ...
        weak_labels = [
            one_hot(np.array(weak_labels_i), n_class=n_class)
            for weak_labels_i in dataset.weak_labels
        ]
        prefix = torch.zeros([1, m, n_class])  # shape: 1, n_src, d_obs
        prefix[:, :, 0] = 1
        self.weak_labels = [
            torch.cat([prefix, torch.FloatTensor(inst)]) for inst in weak_labels
        ]

        pad_embed = torch.zeros((1, d_emb))
        self.embs = [
            torch.cat([pad_embed, torch.FloatTensor(inst)])
            for inst in dataset.bert_embeddings
        ]
...
class CHMM(BaseSeqModel):
    ...
    def fit(
        ...
        **kwargs: Any,
        ...
        config.src_priors = {
            src: {lb: (0.7, 0.7) for lb in dataset_train.entity_types}
            for src in range(m)
        }
        ...
        with torch.no_grad():
            for i, batch in enumerate(test_loader):
                ...
                emb_batch, obs_batch, seq_lens = map(
                    lambda x: x.to(self.device), batch[:3]
                )
                ...
                pred_lbs = [
                    [lb_index for lb_index in label_indices[1:]]
