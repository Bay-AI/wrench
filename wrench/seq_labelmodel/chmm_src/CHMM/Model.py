...
class NeuralModule(nn.Module):
    def __init__(self, d_emb, d_hidden, n_src, d_obs):
    ...
    def forward(self, embs: torch.Tensor, temperature: Optional[int] = 1.0):
    ...
    def _init_parameters(self):
        nn.init.xavier_uniform_(
            self._neural_transition.weight.data, gain=nn.init.calculate_gain("relu")
        )
        for emiss in self._neural_emissions:
            nn.init.xavier_uniform_(
                emiss.weight.data, gain=nn.init.calculate_gain("relu")
            )
...
class CHMM(nn.Module):
    ...
    def _initialize_model(
        ...
        if trans_matrix is None:
            self._unnormalized_trans = nn.Parameter(
                torch.randn(self._n_hidden, self._n_hidden, device=self._device)
            )
        else:
        ...
        if emiss_matrix is None:
            self._unnormalized_emiss = nn.Parameter(
                torch.zeros(
                    self._n_src, self._n_hidden, self._d_obs, device=self._device
                )
            )
        else:
    ...
    def _initialize_states(
        ...
        self._log_state_priors = torch.log_softmax(
            self._state_priors / temperature, dim=-1
        )
        ...
        self._log_trans = torch.log(
            (1 - self._trans_weight) * trans + self._trans_weight * nn_trans
        )
        self._log_emiss = torch.log(
            (1 - self._emiss_weight) * emiss + self._emiss_weight * nn_emiss
        )
        ...
        self._log_emiss_evidence = (
            log_matmul(self._log_emiss, torch.log(obs).unsqueeze(-1))
            .squeeze(-1)
            .sum(dim=-2)
        )

        self._log_alpha = torch.zeros(
            [batch_size, max_seq_length, self._n_hidden], device=self._device
        )
        self._log_beta = torch.zeros(
            [batch_size, max_seq_length, self._n_hidden], device=self._device
        )
        ...
        self._log_xi = torch.zeros(
            [batch_size, max_seq_length, self._n_hidden, self._n_hidden],
            device=self._device,
        )
        return self
    ...
    def _forward_step(self, t):
        ...
        # do the forward step
            ...
            log_alpha_t = self._log_emiss_evidence[:, t, :] + log_matmul(
                self._log_alpha[:, t - 1, :].unsqueeze(1), self._log_trans[:, t, :, :]
            ).squeeze(1)

        # normalize the result
        normalized_log_alpha_t = log_alpha_t - log_alpha_t.logsumexp(
            dim=-1, keepdim=True
        )
    ...
    def _backward_step(self, t):
        ...
        log_beta_t = log_matmul(
            self._log_trans[:, t, :, :],
            (self._log_emiss_evidence[:, t, :] + self._log_beta[:, t + 1, :]).unsqueeze(
                -1
            ),
        ).squeeze(-1)
        return log_beta_t
    ...
    def _forward_backward(self, seq_lengths):
        ...
        self._log_beta = torch.stack(
            [
                torch.roll(beta, s.item(), 0)
                for beta, s in zip(self._log_beta, shift_distances)
            ]
        )
        return None
    ...
    def _compute_xi(self, t):
        ...
        temp_2 = log_matmul(
            self._log_alpha[:, t - 1, :].unsqueeze(-1), temp_1.unsqueeze(1)
        )
    ...
    def _expected_complete_log_likelihood(self, seq_lengths):
        ...
        stabled_norm_term = logsumexp(
            self._log_xi[:, 1:, :, :].view(batch_size, max_seq_length - 1, -1), dim=-1
        ...
        log_prior = torch.sum(
            torch.exp(log_gamma[:, 0, :]) * self._log_state_priors, dim=-1
        )
        log_prior = log_prior.mean()
        # sum over j, k
        log_tran = torch.sum(
            torch.exp(log_xi) * self._log_trans[:, 1:, :, :], dim=[-2, -1]
        )
        # sum over valid time steps, and then average over batch. Note this starts from t=2
        log_tran = torch.mean(
            torch.stack(
                [inst[:length].sum() for inst, length in zip(log_tran, seq_lengths - 1)]
            )
        )
        # same as above
        ...
        log_emis = torch.mean(
            torch.stack(
                [inst[:length].sum() for inst, length in zip(log_emis, seq_lengths)]
            )
        )
    ...
    def forward(self, emb, obs, seq_lengths, normalize_observation=True):
        ...
        self._initialize_states(
            embs=emb, obs=obs, normalize_observation=normalize_observation
        )
    ...
    def viterbi(self, emb, obs, seq_lengths, normalize_observation=True):
        ...
        pre_states = torch.zeros(
            [batch_size, max_seq_length, self._n_hidden],
            dtype=torch.long,
            device=self._device,
        )
        ...
        for t in range(1, max_seq_length):
            ...
            max_log_prob, argmax_val = log_maxmul(
                ...
                self._log_trans[:, t, :, :]
                + self._log_emiss_evidence[:, t, :].unsqueeze(1),
            )
        ...
        norm_marginals = torch.exp(
            log_marginals - logsumexp(log_marginals, dim=-1, keepdim=True)
        )
