...
def batch_prep(
    ...
    emb_batch = torch.stack(
        [
            torch.cat([inst, torch.zeros([max_seq_len - len(inst), d_emb])], dim=-2)
            for inst in emb_list
        ]
    )
    ...
    obs_batch = torch.stack(
        [
            torch.cat([inst, prefix.repeat([max_seq_len - len(inst), 1, 1])])
...
def collate_fn(insts):
    ...
    if len(all_insts) == 4:
        ...
        batch = batch_prep(
            emb_list=embs, obs_list=weak_labels, txt_list=txt, lbs_list=lbs
        )
