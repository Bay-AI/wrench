    ...
    if normal_if:
        ...
        if sample_method == "term":
            ...
            unravel_index = np.unravel_index(
                sort_idx[nor_score_cumsum <= weight_to_remove], shape=if_score.shape
            )
...
class IF(nn.Module):
    def __init__(
        ...
        self.train_dataloader_ = tdata.DataLoader(
            tr_ds, batch_size=len(tr_ds), shuffle=True
        )
        self.valid_dataloader_ = tdata.DataLoader(
    ...
    def generate_x_y(self, w, x, l, y, act_func="identity", return_raw_score=False):
        ...
        if act_func == "identity":
            ...
            ex_label = (
                nor_score.unsqueeze(dim=2).repeat_interleave(self.n_class, dim=-1)
                * self.eye_mat
            )
            ...
            ex_label = (
                y.unsqueeze(dim=2).repeat_interleave(self.n_class, dim=-1)
                * self.eye_mat
            )
            ex_label = ex_label.view(-1, self.n_class)
            if return_raw_score:
    ...
    def generate_renormalize_x_y(self, w, x, l, y, act_func="identity"):
        ...
        raw_score = w[self.w_idx, l + 1, :]
        ex_raw_score = (
            raw_score.unsqueeze(1).expand(-1, self.n_lf * self.n_class, -1, -1).clone()
        )
    ...
    def outter_prod_inverse(self, input_vec):
        ...
        return torch.inverse(
            outter_result.cpu() + self.damp * torch.eye(outter_result.shape[0])
        ).to(input_vec.device)
    ...
    def batch_hvp_v3(self, x, y, params_list, batch_grad_list):
        ...
        one_sample_grad_list = list(
            grad(loss, params_list, create_graph=True, retain_graph=True)
        )

        elemwise_products = 0
        for grad_elem, v_elem in zip(one_sample_grad_list, batch_grad_list):
            elemwise_products = elemwise_products + (
                grad_elem.reshape(
                    -1,
                )
                * v_elem
        ...
        for i in range(self.recursion_depth):
            if mode == "normal":
                ...
                for idx, x, l, y in self.train_dataloader_:
                    ...
                    if mode == "RW":
                        ex_x, ex_label = self.generate_x_y(
                            w, x, l, y, act_func=act_func
                        )
                    elif mode == "WM":
                        ex_x, ex_label = self.generate_renormalize_x_y(
                            w, x, l, y, act_func=act_func
                        )
                    else:
                        raise NotImplementedError

                    batch_hv = self.batch_hvp_v3(
                        ex_x, ex_label, params, batch_h_estimate
                    )
                    batch_temp_list = []
                    for _v, _h_e, _hv in zip(batch_v, batch_h_estimate, batch_hv):
                        _v, _h_e, _hv = _v.detach(), _h_e.detach(), _hv.detach()
    ...
    def compute_hv(self, val_grad_list, mode, w, act_func):
        ...
        for i in range(self.r):
            ...
            s_test_vec_list.append(
                self.batch_s_test(batch_v, batch_h_estimate, mode, w, act_func)
                .detach()
                .unsqueeze(0)
            )
    ...
    def compute_valid_grad_and_hv(self, mode, w, act_func, batch_mode):
        ...
        if batch_mode:
            ...
            for i in trange(len(val_grad_list[0])):
                s_test_vec = self.compute_hv(
                    [v[i] for v in val_grad_list], mode, w, act_func
                )
    ...
    def compute_IF(
        self, if_type, mode="normal", w=None, act_func="identity", batch_mode=False
    ):
        ...
        if if_type == "if":
            ...
            train_if = self.compute_relat_IF(
                mode, w, act_func, batch_mode, return_all=False
            )
        elif if_type == "all":
            train_if = self.compute_relat_IF(
                mode, w, act_func, batch_mode, return_all=True
            )
        else:
            raise NotImplementedError
        ...
        if mode == "normal":
            ...
            if batch_mode:
                ...
                train_if = torch.zeros(
                    self.num_train_samples, self.n_lf, self.n_class, n_test
                ).to(self.device)
            else:
                train_if = torch.zeros(
                    self.num_train_samples, self.n_lf, self.n_class
                ).to(self.device)
            ...
            if mode == "RW":
                if act_func == "identity":
                    for idx, x, l, y in tqdm(self.train_dataloader_):
                        ...
                        with backpack(BatchGrad()):
                            loss.backward()
                            batch_train_grad_vec = (
                                self.model.collect_batch_grad().detach()
                            )

                        if_score = (
                            batch_train_grad_vec.view(
                                -1, self.n_lf, self.n_class, len(s_test_vec)
                            )
                            @ s_test_vec
                        )
                        train_if[idx] = if_score.detach()
                    ...
                    for idx, x, l, y in tqdm(self.train_dataloader_):
                        self.model.zero_grad()
                        ex_x, ex_label, raw_score = self.generate_x_y(
                            w, x, l, y, act_func=act_func, return_raw_score=True
                        )
                        logit = self.model(ex_x)
                        loss = self.model.ce_loss_sum(logit, ex_label)
                        ...
                        if batch_mode:
                            if_score = (batch_train_grad_vec @ s_test_vec).view(
                                -1, self.n_class, n_test
                            ).unsqueeze(1) * raw_score.unsqueeze(-1)
                        else:
                            if_score = (batch_train_grad_vec @ s_test_vec).view(
                                -1, self.n_class
                            ).unsqueeze(1) * raw_score
                        train_if[idx] = (
                            if_score.detach()
                        )  # taylor expension: e^x = 1 + x + o(x)
                ...
                for idx, x, l, y in tqdm(self.train_dataloader_):
                    ...
                    with backpack(BatchGrad()):
                        loss.backward()
                        batch_train_grad_vec = self.model.collect_batch_grad().detach()
                    batch_train_grad_vec = batch_train_grad_vec.reshape(
                        len(x), self.n_lf * self.n_class, -1
                    )
                    if_score = (
                        batch_train_grad_vec.view(
                            -1, self.n_lf, self.n_class, len(s_test_vec)
    ...
    def compute_self_IF(
        self, mode, w=None, act_func="identity", batch_mode=False, return_all=False
    ):
        ...
        if mode == "normal":
            for idx, x, _, y in self.train_dataloader_:
                ...
                for i in tqdm(range(x.shape[0])):
                    self.model.zero_grad()
                    _loss = self.model.ce_loss_sum(
                        logit[i].unsqueeze(0), y[i].unsqueeze(0)
                    )
                    _loss.backward(retain_graph=True)
                    _train_grad = self.model.collect_grad()
                    _train_grad_list = self.model.separate_batch_grad(_train_grad)
                    for j in range(num_layers):
                        inverse_block_diag[j] = inverse_block_diag[
                            j
                        ] + self.outter_prod_inverse(_train_grad_list[j].reshape(-1, 1))
                for j in range(num_layers):
            ...
            for idx, x, l, y in self.train_dataloader_:
                ...
                for i in tqdm(range(ex_x.shape[0])):
                    if ex_label[i].sum() != 0:
                        ...
                        self.model.zero_grad()
                        _loss = self.model.ce_loss_sum(
                            logit[i].unsqueeze(0), ex_label[i].unsqueeze(0)
                        )
                        _loss.backward(retain_graph=True)
                        _train_grad = self.model.collect_grad()
                        _train_grad_list = self.model.separate_batch_grad(_train_grad)
                        for j in range(num_layers):
                            inverse_block_diag[j] = inverse_block_diag[
                                j
                            ] + self.outter_prod_inverse(
                                _train_grad_list[j].reshape(-1, 1)
                            )
                for j in range(num_layers):
        ...
        if mode == "normal":
            ...
            for idx, x, _, y in tqdm(self.train_dataloader_):
                ...
                with backpack(BatchGrad()):
                    ...
                    train_batch_grad_list = self.model.separate_batch_grad(
                        batch_train_grad_vec
                    )
                    temp = 0
                    for i in range(num_layers):
                        temp += (
                            torch.mm(train_batch_grad_list[i], inverse_block_diag[i])
                            * train_batch_grad_list[i]
                        ).sum(dim=-1)
                train_sif[idx] = temp.view(-1, 1)

        else:
            train_sif = torch.zeros(self.num_train_samples, self.n_lf, self.n_class).to(
                self.device
            )
            ...
            if act_func == "identity":
                for idx, x, l, y in tqdm(self.train_dataloader_):
                    ...
                    with backpack(BatchGrad()):
                        ...
                        for i in range(num_layers):
                            temp += (
                                torch.mm(
                                    train_batch_grad_list[i], inverse_block_diag[i]
                                )
                                * train_batch_grad_list[i]
                            ).sum(dim=-1)
                ...
                for idx, x, l, y in tqdm(self.train_dataloader_):
                    ...
                    self.model.zero_grad()
                    logit = self.model(ex_x)
                    finegrained_loss = self.model.ce_loss_sum(
                        logit, raw_score.reshape(-1, self.n_class)
                    )
                    with backpack(BatchGrad()):
                        finegrained_loss.backward(retain_graph=True)
                        batch_train_grad_vec = (
                            self.model.collect_batch_grad()
                            .detach()
                            .view(len(x), self.n_lf, -1)
                        )
